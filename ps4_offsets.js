...
// 13.02 IS 13.00 for the WebKit side - no 13.02 libSceNKWebKit.sprx dump is
// available, and patch-level releases keep the browser binary (same policy as
// 12.52 -> 12.50 above). The kernel half is cross-checked against
// kernel_offset_13.04.js: every key this table carries is identical
// (k_sysent_661 0x110a760, k_jmp_rsi 0x47b31, k_kl_lock 0xe6c20). What moved
// between 13.00 and 13.04 (PRISON0/ROOTVNODE/mmap RWX offsets) is not stored
// here by design - prison0 is read live out of the kernel and the mmap RWX
// offsets live inside the per-fw kpatch blob itself.
//
// Takes patches/1302.bin (generated from the kpatch_shellcode table in
// kernel_offset_13.04.js; differs from 1300.bin in exactly 18 bytes = the
// moved mmap RWX offsets 0x1fa78a/0x1fa78d).
PS4["13.02"] = Object.assign({}, PS4["13.00"], {
    ...
    fw_status: "state=UNTESTED-on-hardware shares=13.00 "
        ...
        + "kernel_rvas=cross-checked-vs-kernel_offset_13.04.js "
        + "(SYSENT_661=0x110a760 JMP_RSI_GADGET=0x47b31 KL_LOCK=0xe6c20) "
        + "kpatch=1302.bin bug=poops",
    kpatch: "1302.bin",
});

// 13.04 - same story as 13.02 (patch release, no separate WebKit dump).
// Kernel values are the 13.04 rows of kernel_offset_13.04.js (13.04 shares
// the offset_ps4_13_02 object), validated to match the shipped 13.04 kpatch
// blob (1304.bin) and the 13.04 mmap offsets [0x1fa79a, 0x1fa79d]. If the
// real 13.04 WebKit ever moved an anchor, stage 1/7 fails loudly and
// harmlessly rather than corrupting anything.
PS4["13.04"] = Object.assign({}, PS4["13.00"], {
    alias_of: "13.00",
    fw_status: "state=UNTESTED-on-hardware shares=13.00 "
        + "webkit=assumed-identical-to-13.00 (no 13.04 module dump) "
        + "kernel_rvas=validated-vs-kernel_offset_13.04.js "
        + "(SYSENT_661=0x110a760 JMP_RSI_GADGET=0x47b31 KL_LOCK=0xe6c20) "
        + "kpatch=1304.bin bug=poops",
    kpatch: "1304.bin",
});
